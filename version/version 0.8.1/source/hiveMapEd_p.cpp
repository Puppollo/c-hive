    ...
    case WM_CHAR ://ascii - wparam, key state - lparam
    {
      switch(wparam)
      {
        ...
        case 'l' : // load map
        {
          ...
          if(load_file)world->LoadLandscape(load_file,0) ;
          fclose(load_file) ;
        } ;
        break;
        case ']' : // next player
...
void GameInit()
{
  ...
  SpriteList->Add(DDraw->DDraw,"art\\terrain.bmp",sprite_TERRAIN_X_FRAME_COUNT,sprite_TERRAIN_Y_FRAME_COUNT*terrain_H_COUNT,log_file) ;         // 1 
...
void GameMain()
{
  ...
  CheckError(res,"font->Draw") ;
  res=font_info->Draw(DDraw->DDSBack,((world->building[mouse.cell_x][mouse.cell_y]).terrain.h),font_info_H_X,font_info_H_Y,1) ;
  CheckError(res,"font->Draw") ;
  res=font_info->Draw(DDraw->DDSBack,world->landscape[mouse.cell_x][mouse.cell_y],font_info_H_X+20,font_info_H_Y,2) ;
  CheckError(res,"font->Draw") ;
