...
struct s_Particicle
{
  s_PInfo PInfo ;
  s_UInfo UInfo ;
  void Move(c_World *world,FILE *log_file) ;
  ...
  if(info.enable==1)
    if(TIME % info.GenerateTime == 0) // генерирование частиц
    {
      ...
      for(int pc=info.isLiveCount;pc<N;pc++)
      {
        ...
        if(particicle[pc].PInfo.dx==0 && particicle[pc].PInfo.dy==0)particicle[pc].PInfo.dx=1;
        particicle[pc].UInfo.param=0;
        particicle[pc].UInfo.SkipEnableCount=0;
        particicle[pc].UInfo.Enable=0;
       } ;
  ...
  if(TIME % info.TurnTime == 0)  // перемещение частиц
  {
    ...
    for(int pc=0;pc<N;pc++)
    {
      ...
      if(particicle[pc].PInfo.isLive)
        particicle[pc].Move(world,log_file) ;
...
//---------------------------------------------------------------------------
void s_Particicle::Move(c_World *world,FILE *log_file)
{
  if(UInfo.SkipEnableCount>0) UInfo.SkipEnableCount--;
  if(UInfo.SkipEnableCount<=0)UInfo.Enable =0 ;
  ...
  for(int x=x_start;x<=x_end;x++)
    for(int y=y_start;y<=y_end;y++)
    {
      ...
      switch((world->building[x][y]).type)
      {
        ...
        {
          int correct_type = (world->building[x][y]).param[PInfo.type+3] ;
          if((world->building[x][y]).enable && correct_type &&
                     ...
                     (world->building[x][y]).r)) // в зоне действия маяка
          {
                       ...
                       building_NODE_WORK_RANGE))
            {
              ...
              UInfo.Enable=1 ;
              UInfo.SkipEnableCount = (world->building[x][y]).param[2];
            }
            else if(UInfo.Enable==0)
              MoveTo(PInfo.x,PInfo.y,(x<<5)+screen_X_CELL_CENTER,(y<<5)+screen_Y_CELL_CENTER) ;
          };
        };
        break ;
        //////////////////////////////////////////////////
        ...
        case building_type_BASE :
          if(cell_x==x && cell_y==y && (world->building[x][y]).enable && PInfo.type!=particicle_type_SUPPLY_EMPTY)
          {
            switch(PInfo.type) // доставка груза
            {
              case particicle_type_SUPPLY_MINERAL :
                if((int)(world->building[x][y]).param[4]<9990)
                {
                  (world->building[x][y]).param[4]+=UInfo.param ;
                  UInfo.param=0;
                };
              break;
              case particicle_type_SUPPLY_ENERGY :
                if((int)(world->building[x][y]).param[5]<9990)
                {
                  (world->building[x][y]).param[5]+=UInfo.param ;
                  UInfo.param=0;
                };
              break;
            } ;
        ...
        case building_type_CONSTRUCTING : // стройка
          if(cell_x==x && cell_y==y && PInfo.type == particicle_type_SUPPLY_EMPTY && (world->building[cell_x][cell_y]).enable) // работать может только "пустая частица"
          {
            ...
            if((world->building[x][y]).param[1]<=0)//проверка конец строительства здания
            {
              ...
              (world->building[x][y]).param[2]=building_exit_coord_X;
              (world->building[x][y]).param[3]=building_exit_coord_Y;
              (world->building[x][y]).param[4]=0;
              (world->building[x][y]).param[5]=0;
              switch((world->building[x][y]).type) // что за здание построено
              {
                case building_type_POWER :
                 (world->building[x][y]).HP = building_POWER_HP ;
                 (world->building[x][y]).maxHP = building_POWER_HP ;
                break;
                case building_type_FACTORY :
                  (world->building[x][y]).param[0]=-1;//завод ничего не производит
                  (world->building[x][y]).param[1]=0;
                  (world->building[x][y]).HP = building_FACTORY_HP ;
                  (world->building[x][y]).maxHP = building_FACTORY_HP ;
                break;
                case building_type_SPEED :
                  (world->building[x][y]).param[0]=1;
                  (world->building[x][y]).param[1]=0;
                  (world->building[x][y]).HP = building_SPEED_HP ;
                  (world->building[x][y]).maxHP = building_SPEED_HP ;
                break;
                case building_type_CHARGE :
                  (world->building[x][y]).HP = building_CHARGE_HP ;
                  (world->building[x][y]).maxHP = building_CHARGE_HP ;
                break;
                case building_type_MINE :
                  (world->building[x][y]).HP = building_MINE_HP ;
                  (world->building[x][y]).maxHP = building_MINE_HP ;
                break;
                //... more buildings
              } ;
            } ;
        ...
        //////////////////////////////////////////////////
        case building_type_MINE :
          if(cell_x==x && cell_y==y && (world->building[x][y]).enable && PInfo.type==particicle_type_SUPPLY_EMPTY)// шахта работать может только "пустая" частица
          {
              ...
              for(int mineral_y=mineral_sy;mineral_y<=mineral_ey;mineral_y++)// поиск минералов вокруг шахты
                if((world->building[mineral_x][mineral_y]).type==building_type_MINERAL)
                {
                  ...
                  if(mineral_resurce_count>0) // месторождение не исчерпано
                  {
                    ...
                    UInfo.param=(world->player[owner]).supply_carring_count;
                    break;
                  }
        ...
        //////////////////////////////////////////////////
        case building_type_POWER :
        if(cell_x==x && cell_y && (world->building[x][y]).enable && PInfo.type==particicle_type_SUPPLY_EMPTY)
          {
