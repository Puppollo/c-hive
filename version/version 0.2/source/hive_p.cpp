...
const int sprite_BUILDINGS_X_COUNT  = 9 ;
...
LRESULT CALLBACK WindowProc(HWND hwnd,UINT msg,WPARAM wparam,LPARAM lparam)
{
  ...
  switch(msg)
  {
    ...
    case WM_MOUSEMOVE :
    {
      mouse.x = (int)LOWORD(lparam) ;
      mouse.y = (int)HIWORD(lparam) ;
      mouse.cell_x = (mouse.x>>5)+current_x_screen ;
      mouse.cell_y = (mouse.y>>5)+current_y_screen ;
...
void GameInit()
{
  ...
  (world->building[6][6]).y_frame = building_y_frame_NODE_U ;
  (world->building[6][6]).r = building_NODE_RANGE ;
  (world->building[6][6]).param[0] = 0 ;
  (world->building[6][6]).param[1] =-2 ;
  (world->building[6][6]).param[2] = 0 ;
  (world->building[6][6]).param[3] = building_NODE_RANGE+1 ;

  (world->building[4][4]).type = building_type_MAIN ;
...
void GameMain()
{
  ...
  if(ThisTickCount-MouseScrollTickCount>map_scroll_DELAY)
  {
    ...
    MoveMapByMouse(mouse.x,mouse.y,current_x_screen,current_y_screen,screen_HEIGHT,screen_WIDTH) ;
  } ;
  //drawind
  ...
  CheckError(res,"font->Draw") ;
  res = font->Draw(DDraw->DDSBack,HiveElements->GetScreenPanel(),600,210) ;
  CheckError(res,"font->Draw") ;
  //other
  world->TIME++;
