  ...
  for(int x=x_start;x<x_end;x++)
    for(int y=y_start;y<y_end;y++)
    {
      ...
      switch(building[x][y].type)
      {
          ...
          if(cell_x==x && cell_y==y && (building[x][y]).enable && PInfo.type!=particicle_type_SUPPLY_EMPTY)
          {
            switch(PInfo.type) // доставка груза
            {
              case particicle_type_SUPPLY_MINERAL :
                if((int)building[x][y].param[4]<building_MAX_MINERAL_COUNT)
                {
              ...
              case particicle_type_SUPPLY_ENERGY :
                if((int)building[x][y].param[5]<building_MAX_ENERGY_COUNT)
                {
            ...
            if(building[x][y].param[1]<=0)//проверка конец строительства здания
            {
              ...
              switch((building[x][y]).type) // что за здание построено
              {
                ...
                case building_type_SEPARATOR :
                  (building[x][y]).HP = building_SEPARATOR_HP ;
                  (building[x][y]).maxHP = building_SEPARATOR_HP ;
                  (building[x][y]).y_frame = building_y_frame_SEPARATOR_HORIZONTAL ;
                  (building[x][y]).param[0]=0;
                break;
                //... more buildings
              } ;
            } ;
              ...
              for(int mineral_y=mineral_sy;mineral_y<=mineral_ey;mineral_y++)// поиск минералов вокруг шахты
                if((building[mineral_x][mineral_y]).type==building_type_MINERAL)
                {
                  ...
                  if(mineral_resurce_count>0) // месторождение не исчерпано
                  {
                    ...
                    UInfo.param=(player[owner]).supply_mineral_carring_count;
                    int dx=(building[x][y].param[2]),dy=(building[x][y]).param[3];
                    PInfo.x = ((x+dx)<<5)+screen_X_CELL_CENTER ;
                    PInfo.y = ((y+dy)<<5)+screen_Y_CELL_CENTER ;
                    UInfo.SkipEnableCount=0;
        ...
        case building_type_POWER :
          if(cell_x==x && cell_y==y && (building[x][y]).enable &&
             building[x][y].terrain.height>world_ENERGY_HEIGHT_START && PInfo.type==particicle_type_SUPPLY_EMPTY)
          {
            PInfo.type = particicle_type_SUPPLY_ENERGY ;
            UInfo.param = ((player[owner]).supply_energy_carring_count)*(building[x][y].terrain.height-world_ENERGY_HEIGHT_START) ;
            int dx=(building[x][y].param[2]),dy=(building[x][y]).param[3];
            PInfo.x = ((x+dx)<<5)+screen_X_CELL_CENTER ;
            PInfo.y = ((y+dy)<<5)+screen_Y_CELL_CENTER ;
            ...
            UInfo.SkipEnableCount=0;
            UInfo.ChangeDirection_flag=1;
            building[x][y].terrain.dheight-- ;
            if(((-1)*(building[x][y].terrain.dheight))>building_terrain_MAX_DHEIGHT)
              building[x][y].terrain.dheight=-building_terrain_MAX_DHEIGHT ;
            return ;
          } ;
        break;
        //////////////////////////////////////////////////
        case building_type_SEPARATOR :
        {
          ...
          if(cell_x==x && cell_y==y && correct_type && building[x][y].enable)
          {
            int a=0;
            if(building[x][y].param[0]==0)
            {
              building[x][y].param[0]=1 ;
              a=1 ;
            }
            ...
            switch(building[x][y].y_frame)
            {
              case building_y_frame_SEPARATOR_HORIZONTAL :
                PInfo.x =((x+a)<<5)+screen_X_CELL_CENTER;
                PInfo.y =(y<<5)+screen_Y_CELL_CENTER;
              break;
              case building_y_frame_SEPARATOR_VERTICAL :
                PInfo.x=(x<<5)+screen_X_CELL_CENTER ;
                PInfo.y=((y+a)<<5)+screen_Y_CELL_CENTER ;
              break;
            } ;
            UInfo.SkipEnableCount=0;
            UInfo.ChangeDirection_flag=1;
        ...
        break;
        //////////////////////////////////////////////////
        case building_type_BLOW :
          if(cell_x==x && cell_y==y)
          {
            PInfo.CurrentTurn+=building_BLOW_ENERGY_REDUCED ;
          } ;
        //////////////////////////////////////////////////
