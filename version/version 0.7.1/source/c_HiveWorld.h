...
class c_World:public c_grObject
{
  private :
    ...
    //void GenHeight() ;
    void GenMineral() ; // расширение минеральных жил
  public :
    int TIME ,CYCLE;
    s_Player *player;
  ...
  for(int x=0;x<world_X_COUNT;x++)
    for(int y=0;y<world_Y_COUNT;y++)
      if(building[x][y].type==building_type_MINERAL)
      {
        if(x>0)
        {
          int r=random(100*(building[x-1][y].terrain.height+1)) ;
        ...
        if(y>0)
        {
          int r=random(100*(building[x][y-1].terrain.height+1)) ;
          if(r<world_MINERAL_IN_RANDOM)
          {
...
void c_World::GenMap()
{
  ...
  GenMineral() ; // расширение минеральных жил
} ;
//---------------------------------------------------------------------------
...
void c_World::Turn(FILE *log_file)
{
  ...
  if(TIME>world_CHANGE_CYCLE)
  {
    TIME=0;
    CYCLE++;
    if(CYCLE % world_CYCLE_MINERAL==0)GenMineral() ;
  };
