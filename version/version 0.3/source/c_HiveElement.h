...
{
  private :
    void (*OnClickFunction)(s_mouse *mouse,c_World *world,int &action,c_Element *element) ;
    void (*OnMoveFunction)(s_mouse *mouse,c_World *world,int &action,c_Element *element) ;
    int visible ;
    int enable ;
    int x,y,width,height ;
  public :
    c_Element(c_Sprite *s,int xx,int yy,int w=0,int h=0,int fx=0,int fy=0,
              void (*OnClickF)(s_mouse *mouse,c_World *world,int &action,c_Element *element)=NULL,
              void (*OnMoveF)(s_mouse *mouse,c_World *world,int &action,c_Element *element)=NULL,
              FILE *log_file=NULL) ;
    int CheckState(s_mouse *mouse,c_World *world,int &action) ;
...
c_Element::c_Element(c_Sprite *s,int xx,int yy,int w,int h,int fx,int fy,
                     void (*OnClickF)(s_mouse *mouse,c_World *world,int &action,c_Element *element),
                     void (*OnMoveF)(s_mouse *mouse,c_World *world,int &action,c_Element *element),
                     FILE *log_file)
:c_grObject(s,xx,yy)
{
  x=xx;y=yy;
...
//---------------------------------------------------------------------------
int c_Element::CheckState(s_mouse *mouse,c_World *world,int &action)
{
  if(enable)
    if(mouse->x>x && mouse->x<x+width && mouse->y>y && mouse->y<y+height)
    {
      OnMoveFunction(mouse,world,action,this) ;
      if(mouse->button!=mouse_button_NO)
        OnClickFunction(mouse,world,action,this);
    } ;
} ;
//---------------------------------------------------------------------------
#endif
