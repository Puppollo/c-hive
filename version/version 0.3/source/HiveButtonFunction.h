...
const int action_BUILD_NEXT       = 1 ; // нажатие на кнопку след. здание
const int action_BUILD_PREV       = 2 ; // нажатие на кнопку перд. здание
...
const int action_BUILDING_CLICK   =19 ; // нажатие на здание
const int action_DESTROY_BUILDING =20 ; // уничтожить здание
const int action_SHOW_TERRITORY   =21 ; // показать территорию
...
void OnDestroyBuilding_click(s_mouse *mouse,c_World *world,int &action,c_Element *element) ; // уничтожить здание

void OnShowTerritory_click(s_mouse *mouse,c_World *world,int &action,c_Element *element) ;// показать территорию
//... more function
//---------------------------------------------------------------------------
...
void c_HiveElements::Turn(char key,FILE *log_file)
{
  ...
  switch(common_screen_panel)
  {
    case screen_panel_COMMON_BUILDING :
      emtDestroyBuilding->CheckState(mouse,world,action) ;
      //...
    break;
    case screen_panel_COMMON_NO :
      emtShowTerritory->CheckState(mouse,world,action) ;
      //...
    break;
  } ;
...
//---------------------------------------------------------------------------
void OnDestroyBuilding_click(s_mouse *mouse,c_World *world,int &action,c_Element *element)
{
  action = action_DESTROY_BUILDING ;
  element->SetFrameY(emtDestroyBuilding_FRAME_Y+1) ;
}
//---------------------------------------------------------------------------
void OnShowTerritory_click(s_mouse *mouse,c_World *world,int &action,c_Element *element)
{
